...
export default async function FullReportPage({ params }: FullPageProps) {
        ...
        <div className="flex h-full flex-col justify-between">
          <div className="space-y-10">
            <div className="space-y-4">
              ...
              <p className="max-w-3xl text-base text-black/70 md:text-lg">{report.hookSummary}</p>
            </div>

            <div className="grid gap-4 md:grid-cols-[1.1fr_0.9fr]">
              <div className="rounded-[2rem] bg-[#111111] p-7 text-white">
                <p className="text-xs uppercase tracking-[0.25em] text-white/55">resumen ejecutivo</p>
              ...
              <div className="grid gap-4">
                ...
                <div className="rounded-[2rem] border border-black/10 bg-white/75 p-6">
                  <p className="text-xs uppercase tracking-[0.2em] text-black/45">salud actual</p>
            ...
            <div className="grid gap-4 md:grid-cols-4">
              ...
              <InfoCard
                label="Estabilidad"
                value={formatCLS(mobile?.cumulativeLayoutShift ?? gtmetrix?.cumulativeLayoutShift ?? null)}
          ...
          <div>
            ...
            <div className="mt-8 grid gap-5">
              {report.healthRows.map((row) => (
                <div key={row.metric} className="rounded-[1.75rem] border border-black/10 bg-white/75 p-6">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      <p className="text-xs uppercase tracking-[0.2em] text-black/45">métrica</p>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-3">
              <InfoCard
                label="LCP"
                value={formatMilliseconds(mobile?.largestContentfulPaint ?? gtmetrix?.largestContentfulPaint ?? null)}
                helper="Tiempo hasta el contenido principal"
              />
              <InfoCard
                label="TBT"
                value={formatMilliseconds(mobile?.totalBlockingTime ?? gtmetrix?.totalBlockingTime ?? null)}
      ...
      <section className="report-page report-card mt-6 rounded-[2.25rem] p-8 md:p-12">
        <div className="flex h-full flex-col justify-between">
          <div className="grid gap-6 md:grid-cols-[1.02fr_0.98fr]">
            <div className="rounded-[2rem] bg-[#111111] p-7 text-white">
              <p className="text-xs uppercase tracking-[0.25em] text-white/55">hallazgos clave</p>
              <div className="mt-6 space-y-5">
                {report.findings.map((finding) => (
                  <div key={finding} className="rounded-[1.5rem] border border-white/10 bg-white/5 p-5">
            ...
            <div className="rounded-[2rem] border border-black/10 bg-white/75 p-7">
              <p className="text-xs uppercase tracking-[0.25em] text-black/45">el costo de no actuar</p>
              <p className="mt-6 text-2xl leading-tight text-black">{report.costOfInaction}</p>

              <div className="mt-8 rounded-[1.5rem] border border-black/10 bg-black px-6 py-5 text-white">
                <p className="text-xs uppercase tracking-[0.2em] text-white/55">lectura técnica</p>
        ...
        <div className="flex h-full flex-col justify-between">
          <div className="grid gap-6 md:grid-cols-[0.95fr_1.05fr]">
            <div className="rounded-[2rem] bg-[#1F00FF] p-7 text-white">
              <p className="text-xs uppercase tracking-[0.25em] text-white/60">propuesta de optimización rápida</p>
              <div className="mt-6 space-y-4">
                {report.proposals.map((proposal) => (
                  <div key={proposal} className="rounded-[1.5rem] border border-white/15 bg-white/5 p-5">
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-[2rem] border border-black/10 bg-white/75 p-7">
                <p className="text-xs uppercase tracking-[0.25em] text-black/45">siguiente paso sugerido</p>
                <p className="mt-4 text-2xl leading-tight text-black">
                  Convertir este diagnóstico en una hoja de ruta de implementación rápida con foco
                  en percepción, velocidad y conversión.
                </p>
              </div>
            </div>
          </div>
